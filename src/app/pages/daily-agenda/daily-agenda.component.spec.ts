...
describe('DailyAgendaComponent', () => {
  ...
  describe('BUG 2 - students loading per class group', () => {
    it('loads only students from the specific class group when expanding card', () => {
      const classGroupA = 'cg-class-a';
      const classGroupB = 'cg-class-b';

      const enrollmentA: Enrollment = { id: 'enr-a1', studentId: 's-a1', studentName: 'Aluno A1', classGroupId: classGroupA, status: 'ACTIVE', enrollmentDate: '2026-01-01' };
      const enrollmentA2: Enrollment = { id: 'enr-a2', studentId: 's-a2', studentName: 'Aluno A2', classGroupId: classGroupA, status: 'ACTIVE', enrollmentDate: '2026-01-01' };
      const enrollmentB: Enrollment = { id: 'enr-b1', studentId: 's-b1', studentName: 'Aluno B1', classGroupId: classGroupB, status: 'ACTIVE', enrollmentDate: '2026-01-01' };

      enrollmentService.getStudentsByClassGroupId.calls.reset();

      enrollmentService.getStudentsByClassGroupId.withArgs(classGroupA).and.returnValue(of([enrollmentA, enrollmentA2]));
      enrollmentService.getStudentsByClassGroupId.withArgs(classGroupB).and.returnValue(of([enrollmentB]));

      const cardA: any = {
        session: { ...mockSession, id: 'session-a', title: 'Turma A' },
      ...
      const cardB: any = {
        session: { ...mockSession, id: 'session-b', title: 'Turma B', scheduledDate: todayStr() },
        instructorName: 'Instrutor',
      ...
      component.toggleCard(cardA);
      fixture.detectChanges();

      expect(enrollmentService.getStudentsByClassGroupId).toHaveBeenCalledWith(classGroupA);
      expect(cardA._students?.length).toBe(2);
      expect(cardA._students?.some((s: any) => s.studentId === 's-b1')).toBeFalse();
    });

    it('does NOT call getAll enrollments to avoid loading all students', () => {
      expect(enrollmentService.getAll).not.toHaveBeenCalledWith(jasmine.objectContaining({ classGroupId: 'cg-1' }));
    });

    it('does NOT show students from other class groups in expanded card', () => {
      const classGroupAStudents = [
        { studentId: 's-a1', studentName: 'Aluno Turma A', enrollmentStatus: 'ACTIVE' },
      ];
      const classGroupBStudents = [
        { studentId: 's-b1', studentName: 'Aluno Turma B', enrollmentStatus: 'ACTIVE' },
      ];

      enrollmentService.getStudentsByClassGroupId.calls.reset();
      enrollmentService.getStudentsByClassGroupId.and.callFake((classGroupId: string) => {
        if (classGroupId === 'cg-class-a') return of(classGroupAStudents.map(s => ({
          id: s.studentId, studentId: s.studentId, studentName: s.studentName, classGroupId: 'cg-class-a', status: 'ACTIVE', enrollmentDate: '2026-01-01'
        })));
        if (classGroupId === 'cg-class-b') return of(classGroupBStudents.map(s => ({
          id: s.studentId, studentId: s.studentId, studentName: s.studentName, classGroupId: 'cg-class-b', status: 'ACTIVE', enrollmentDate: '2026-01-01'
        })));
        return of([]);
      });
      ...
      expect(cardA._students?.length).toBe(1);
      expect(cardA._students?.[0].studentName).toBe('Aluno Turma A');
    });
  });
