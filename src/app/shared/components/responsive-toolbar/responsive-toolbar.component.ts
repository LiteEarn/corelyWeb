...
@Component({
  ...
  styles: [`
    ...
    @media (max-width: 959px) {
      .responsive-toolbar { flex-direction: column; gap: 12px; }
    }
    .toolbar-search { flex: 1; min-width: 240px; }
    @media (max-width: 959px) {
      .toolbar-search { width: 100%; min-width: unset; }
    }
    ...
    .search-field ::ng-deep .mat-mdc-form-field-infix { padding: 12px 16px; min-height: 48px; }
    .search-icon { color: #94A3B8; font-size: 20px; }
    .toolbar-filters { display: flex; gap: 12px; align-items: center; flex-wrap: wrap; }
    .toolbar-filters--stacked { width: 100%; flex-direction: column; align-items: stretch; }
    .toolbar-actions { display: flex; align-items: center; }
    @media (max-width: 959px) { .toolbar-actions { display: none; } }
    .new-button { white-space: nowrap; height: 48px; }
    .crud-fab {
      position: fixed; bottom: 24px; right: 24px; z-index: 1030;
    }
    @media (min-width: 960px) { .crud-fab { display: none !important; } }
  `]
})
